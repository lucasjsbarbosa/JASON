...
def train(
    *,
    ...
    rho = _spearman(y_va, y_pred)
    pair_acc = _pairwise_intra_bucket_accuracy(y_va, y_pred, val_df["subs_bucket"].values)

    importance = dict(sorted(
        zip(booster.feature_name(), booster.feature_importance(), strict=True),
        key=lambda kv: kv[1], reverse=True,
    ))

    booster.save_model(str(artifacts / "model.lgb"))
    ...
    logger.info("trained on %d / val %d, spearman=%.3f, pair_acc=%.3f",
                len(train_df), len(val_df), rho, pair_acc)
