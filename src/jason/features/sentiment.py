"""Sentiment scoring de títulos via pysentimiento (PT-BR).
...
def _read_pending(
    ...
    *,
    ...
    """Returns (video_id, title) for videos missing sentiment_score (or all if force)."""
    sql = ["SELECT v.id, v.title FROM videos v"]
    sql.append("LEFT JOIN video_features f ON f.video_id = v.id")
    if force:
        sql.append("WHERE 1=1")
    else:
        sql.append("WHERE (f.video_id IS NULL OR f.sentiment_score IS NULL)")
    params: list[Any] = []
    if channel_id:
